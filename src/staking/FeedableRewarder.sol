...
contract FeedableRewarder is IRewarder, OwnableUpgradeable {
  ...
  event LogUpdateRewardCalculationParams(
    uint64 lastRewardTime, uint256 accRewardPerShare
  );
  event LogFeed(
    uint256 feedAmount, uint256 rewardRate, uint256 rewardRateExpiredAt
  );
    ...
    onlyStakingContract
  {
    ...
    userRewardDebts[user] = userRewardDebts[user]
    ...
    onlyStakingContract
  {
    ...
    int256 accumulatedRewards =
      ((_userShare(user) * accRewardPerShare) / ACC_REWARD_PRECISION).toInt256();
    uint256 pendingRewardAmount =
      (accumulatedRewards - userRewardDebts[user]).toUint256();
  ...
  function pendingReward(address user) external view returns (uint256) {
    uint256 projectedAccRewardPerShare =
      accRewardPerShare + _calculateAccRewardPerShare(_totalShare());
    int256 accumulatedRewards = (
      (_userShare(user) * projectedAccRewardPerShare) / ACC_REWARD_PRECISION
    ).toInt256();
  ...
  function _feed(uint256 feedAmount, uint256 duration) internal {
...

    {
      ...
      IERC20Upgradeable(rewardToken).safeTransferFrom(
        msg.sender, address(this), feedAmount
      );

      if (
        IERC20Upgradeable(rewardToken).balanceOf(address(this)) - balanceBefore
          != feedAmount
