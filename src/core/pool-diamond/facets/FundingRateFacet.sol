...
contract FundingRateFacet is FundingRateFacetInterface {
  ...
  event UpdateFundingRate(
    address token, int256 sumRateLong, int256 sumRateShort
  );
  ...
  function _updateBorrowingRateAndFundingRate(address token) internal {
    ...
    if (poolV1ds.lastFundingTimeOf[token] == 0) {
      poolV1ds.lastFundingTimeOf[token] =
        (block.timestamp / fundingInterval) * fundingInterval;
      return;
    }
    ...
    if (poolV1ds.lastFundingTimeOf[token] + fundingInterval <= block.timestamp)
    {
      ...
      uint256 borrowingRate =
        GetterFacetInterface(address(this)).getNextBorrowingRate(token);
      unchecked {
      ...
      (int256 fundingRateLong, int256 fundingRateShort) =
        GetterFacetInterface(address(this)).getNextFundingRate(token);
      unchecked {
