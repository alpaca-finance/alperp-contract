...
contract AP_Mint is AP_BaseTest {
  ...
  function _getAccountAmount(uint256 _weekTimestamp, address _owner)
    internal
    returns (uint256)
  {
    (uint256 amount, ) = ap.weeklyAccountAmount(_weekTimestamp, _owner);
  ...
  function testCorrectness_Mint() external {
    ...
    assertEq(_getAccountAmount(0, BOB), 1 ether);
    ...
    assertEq(_getAccountAmount(2773, BOB), 0);
    assertEq(_getAccountAmount(0, CAT), 0);
    assertEq(_getAccountAmount(2773, CAT), 1.5 ether);
    ...
    assertEq(_getAccountAmount(0, BOB), 1 ether);
    assertEq(_getAccountAmount(2773, BOB), 0);
    assertEq(_getAccountAmount(2774, BOB), 2 ether);
    assertEq(_getAccountAmount(0, CAT), 0);
    assertEq(_getAccountAmount(2773, CAT), 1.5 ether);
    assertEq(_getAccountAmount(2774, CAT), 0.5 ether);
  }
