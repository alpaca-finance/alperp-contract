...
contract FeedableRewarderTest is BaseTest {
  ...
  function setUp() external {
    ...
    rewardToken = new MockErc20("ALPACA Token", "ALPACA", 18);
    rewarder = deployFeedableRewarder(
      "ALPACARewarder", address(rewardToken), address(mockStaking)
    );
  ...
  function testCorrectness_WhenRewarderIsInit() external {
    assertEq(rewarder.name(), "ALPACARewarder");
  ...
  function testCorrectness_WhenRewarderOnDepositIsHooked() external {
    ...
    assertCloseWei(
      rewarder.accRewardPerShare(), 135.28138528 ether, 0.00000001 ether
    );
    // 188 * 135.28138528 * 1e18 / 1e20 = 254.32900433
    assertCloseWei(
      rewarder.userRewardDebts(ALICE), 254.32900433 ether, 0.00000001 ether
    );
    ...
    assertCloseWei(
      rewarder.accRewardPerShare(), 264.6809712 ether, 0.00000001 ether
    );
    // 254.329004328844 + (23 * 2.646809712026) = 315.205627705442
    assertCloseWei(
      rewarder.userRewardDebts(ALICE), 315.205627705442 ether, 0.00000001 ether
    );
  }
  ...
  function testCorrectness_WhenAliceAndBobDeposit() external {
    ...
    assertCloseWei(
      rewardToken.balanceOf(ALICE), 14999.99999999998 ether, 0.00000001 ether
    );
    assertCloseWei(
      rewardToken.balanceOf(BOB), 4999.99999999999 ether, 0.00000001 ether
    );
  }
  ...
  function testCorrectness_WhenAliceAndBobDeposit_ThenAliceWithdraw() external {
    ...
    mockStaking.harvest(address(rewarder), BOB);
    assertCloseWei(
      rewardToken.balanceOf(ALICE), 9999.99999999998 ether, 0.00000001 ether
    );
    assertCloseWei(
      rewardToken.balanceOf(BOB), 9999.99999999998 ether, 0.00000001 ether
    );
  }
    ...
    external
  {
    ...
    mockStaking.harvest(address(rewarder), ALICE);
    assertCloseWei(
      rewardToken.balanceOf(ALICE), 19999.99999999998 ether, 0.00000001 ether
    );
  }
  ...
  function test_WhenFeedTokenMultipleTimes() external {
    ...
    assertEq(rewardToken.balanceOf(ALICE), 0);
    mockStaking.harvest(address(rewarder), ALICE);
    assertEq(rewardToken.balanceOf(ALICE), 24679.9999999999993824 ether);
  }
