...
contract PoolDiamond_Farm_IncreasePositionTest is PoolDiamond_BaseTest {
    ...
    external
  {
    ...
    assertEq(
      poolGetterFacet.alp().balanceOf(ALICE), 92.573912195121951219 ether
    );
    ...
    (bool isProfit, uint256 strategyDelta) =
      poolGetterFacet.getStrategyDeltaOf(address(wbtc));
    ...
    poolPerpTradeFacet.increasePosition(
      ALICE, 0, address(wbtc), address(wbtc), 49 * 10 ** 30, true
    );
    ...
    external
  {
    ...
    poolPerpTradeFacet.increasePosition(
      ALICE, 0, address(dai), address(wbtc), 90 * 10 ** 30, false
    );
    ...
    assertEq(
      poolGetterFacet.getRedemptionCollateralUsd(address(dai)), 519.8 * 10 ** 30
    );
